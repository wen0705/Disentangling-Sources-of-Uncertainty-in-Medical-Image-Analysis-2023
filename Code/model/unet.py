...
import wandb
from model.unet_compute import compute_eval_loss,compute_train_loss_and_train
...
class Unet(nn.Module):
    ...
    def loss(self,mask):
        ...
        _, segmentation = torch.max(self.prediction, dim=1)
...
def train_model(model, train_loader, eval_loader, optim, epochs=1, wandb_name=None, save_model=None, save_path=None):
    ...
    for epoch in range(epochs):
        ...
        train_running_loss,diceT,seggT,maskT,oriT = compute_train_loss_and_train(train_loader, model, optim, use_gpu=use_gpu)  
    ...
    # Testing
        # if epoch > 120:
    eval_running_loss,diceE,seggE,maskE,oriE = compute_eval_loss(eval_loader, model, optim, use_gpu=use_gpu)
    test_dice.append(diceE)
    # test_ged.append(gedE)

    visual_test(seggE,maskE,test_dice,oriE,maskE,maskE,maskE,eval_running_loss,0,0,0,0,0,0)

    print("average testing dice:", np.mean(test_dice))  
    # print("average testing ged:", np.mean(test_ged))
    print("average training dice:", np.mean(train_dice))
    # print("average training ged:", np.mean(train_ged))
    return
